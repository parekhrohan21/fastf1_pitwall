...
- **Corner-by-Corner Analysis**: An advanced performance tab that fetches track layout coordinates via FastF1 to let you select a corner (e.g. Turn 1). Automatically calculates apex speed, braking points (with a deceleration fallback if telemetry is missing), and distance-to-apex, plotting both the racing lines and the speed comparison profiles in a dual subplot layout.
