...
| `src/ui/components.py` | Contains all Streamlit UI cards, weather grids, pit stop/ideal lap section details, teammate battle section (`_render_teammate_battle_section`), pit lane transit loss breakdown section (`_render_pit_loss_section`), final official classification tables, consistency section (`_render_consistency_section`), weather correlation section (`_render_weather_correlation_section`), multi-year comparison section (`_render_multi_year_comparison_section`), tyre crossover prediction matrix (`render_tyre_crossover_matrix`), fuel-decoupled degradation metrics (`render_fuel_decoupled_deg_metrics`), grid heatmap section (`_render_grid_heatmap_section`), PDF export section (`render_export_section`), multi-format telemetry export panel (`render_telemetry_export_panel`), braking dynamics section (`_render_braking_analysis_section`), gear shift strategy section (`_render_gear_analysis_section`), speed trap breakdown section (`_render_speed_trap_section`), layout maps block tabs, and footer. |
| `src/charts/plotly.py` | Constructs and returns interactive Plotly figure objects for lap history, tyre strategy Gantt timelines, gap analysis, track maps, tyre degradation (linear + quadratic thermal curves + cliff vlines + fuel burn decoupling), teammate battle diverging delta charts (`build_teammate_matrix_fig`), pit transit loss stacked horizontal bar charts (`build_pit_loss_fig`), stint consistency violin/boxplots (`build_stint_consistency_fig`), weather correlation dual-axis (`build_weather_correlation_fig`), multi-year comparison speed delta (`build_multi_year_comparison_fig`), animated replays, 4-subplot corner analysis (`build_corner_fig`), 3-subplot braking efficiency overlays (`build_braking_efficiency_fig`), 2-subplot gear shift & RPM distributions (`build_gear_shift_fig`), 4-axis speed trap polar radars (`build_speed_trap_radar_fig`), and constructor/PU grouped bar benchmarks (`build_speed_trap_bar_fig`). |
| `src/charts/matplotlib.py` | Creates static Matplotlib figures for 6-channel telemetry profiles and speed delta overlays. |
| `tests/` | Pytest unit and integration tests (95 tests across 16 modules, e.g. `test_pit_transit_loss.py`, `test_teammate_battle.py`, `test_fuel_decoupled_tyre_deg.py`, `test_speed_trap.py`, `test_gear_shifts.py`, `test_braking_analysis.py`, `test_telemetry_export.py`, `test_telemetry_channels.py`, `test_tyre_crossover.py`, `test_corner_analysis.py`). |
...
- **Decision #33**: *Braking Dynamics & Trail-Braking Zone Analysis (`_calculate_braking_metrics`, `build_braking_efficiency_fig`, `_render_braking_analysis_section`)* — Added dedicated braking dynamics and trail-braking analytics around circuit turn apexes. Slices telemetry in a $[d_{\text{apex}} - 350, d_{\text{apex}} + 100]$ window, computing longitudinal deceleration in G-force ($G = -\frac{\Delta v}{\Delta t \cdot 9.81}$) with 3-point moving average smoothing. Extracts Initial Braking Distance, Peak Deceleration, Trail-Braking Release Point, Trail-Braking Distance, and Brake-to-Throttle Transition Time (ms). Renders a 3-row stacked Plotly figure (`build_braking_efficiency_fig`) and comparison cards (`_render_braking_analysis_section`) with later-braking advantage callouts and full `_fmt_driver` formatting.
- **Decision #34**: *Gear Shift Strategy & RPM Power Band Optimization (`_calculate_gear_shift_metrics`, `build_gear_shift_fig`, `_render_gear_analysis_section`)* — Added powertrain and transmission strategy analytics. Evaluates upshift and downshift events, tactical short-shifts (< 11,000 RPM under > 60% throttle), redline shifts (≥ 11,800 RPM), and percentage gear usage distribution (Gears 1 through 8). Renders a dual-subplot Plotly figure (`build_gear_shift_fig`) with RPM operating curve, shift event markers, and horizontal gear distribution bars, alongside 4 metric cards and automated driver comparison summaries in `_render_gear_analysis_section`.
- **Decision #35**: *Speed Trap & Intermediate Velocity Radar Breakdown (`_calculate_speed_trap_metrics`, `build_speed_trap_radar_fig`, `build_speed_trap_bar_fig`, `_render_speed_trap_section`)* — Added grid-wide speed trap and intermediate micro-sector velocity analytics. Extracts maximum velocities at `SpeedST`, `SpeedI1`, `SpeedI2`, and `SpeedFL`. Calculates DRS aerodynamic boost deltas ($\Delta \text{km/h}$ between DRS and non-DRS speed trap entries) and maps constructors to official Power Unit manufacturers (`Ferrari`, `Mercedes`, `Red Bull Powertrains`, `Renault`). Renders a 4-axis polar radar chart (`build_speed_trap_radar_fig`) with driver overlays and grid maximum baseline, grouped constructor/engine benchmark bar charts (`build_speed_trap_bar_fig`), 5 top metric cards, and a classified Speed Trap Leaderboard table with driver highlighting.
- **Decision #36**: *Fuel-Corrected Pure Tyre Degradation & Fuel Burn Decoupler (`_build_fuel_decoupled_tyre_deg`, `render_fuel_decoupled_deg_metrics`)* — Added pure tyre wear decoupling by removing race fuel burn mass gains (~0.3 kg/lap ≈ 0.035 s/lap) from lap times ($t_{\text{corrected}} = t_{\text{lap}} - \alpha \cdot (\text{TotalLaps} - \text{LapNumber})$). Fits linear OLS regression and degree-2 quadratic polynomial curves on decoupled pace to calculate True Degradation Rate ($\Delta\text{s/lap}$) and unmasked thermal cliff laps. Updated `build_tyre_deg_fig` with dynamic fuel decoupling support, dual pace hover tooltips (True Pace, Raw Lap Time, Fuel Offset), and decoupled table statistics. Added `render_fuel_decoupled_deg_metrics` in `src/ui/components.py` (True Deg Rate, Raw Deg Rate, Fuel Effect, Fuel Masking Offset) and enhanced `render_tyre_crossover_matrix` with true vs raw rate indicators. In `app.py`, added interactive fuel decoupling toggle and sensitivity slider (0.010–0.070 s/lap). Automated test suite expanded to 79 tests across 14 modules.
- **Decision #37**: *Intra-Team Teammate Battle & Qualifying Delta Matrix (`_build_teammate_battle_data`, `build_teammate_matrix_fig`, `_render_teammate_battle_section`)* — Added automated intra-team teammate comparison engine across all constructors for Qualifying and Race sessions. Pairs drivers by team from official session results, extracting qualifying personal best lap times, delta ($\Delta\text{s}$ and $\Delta\%$), sector splits (S1/S2/S3 personal bests and micro-sector dominance count), and clean-air median race pace (excluding in-laps, out-laps, safety car periods, and laps >107% of driver median). Visualises a grid-wide horizontal diverging bar chart coloured by official constructor liveries with parity zero line (`build_teammate_matrix_fig`), 4 summary KPI cards (Closest battle, Largest delta, Grid median gap, Sector dominance leader), and an interactive classified Head-to-Head table (`_render_teammate_battle_section`). Automated test suite expanded to 87 tests across 15 modules.
- **Decision #38**: *Pit Lane Transit Loss & In-Lap / Out-Lap Performance Breakdown (`_build_pit_transit_data`, `build_pit_loss_fig`, `_render_pit_loss_section`)* — Added automated telemetry breakdown of pit lane time losses across all drivers and pit stops. Calculates pit lane speed-limiter transit duration ($t_{\text{pit\_lane}} = \text{PitOutTime} - \text{PitInTime}$), in-lap push delta against clean-air baseline flyer pace ($\Delta t_{\text{in}} = t_{\text{in}} - t_{\text{baseline}}$), out-lap cold tyre warm-up performance ($\Delta t_{\text{out}} = t_{\text{out}} - t_{\text{baseline}}$), net total pit loss ($\Delta t_{\text{total\_pit}} = (t_{\text{in}} + t_{\text{out}}) - 2 \cdot t_{\text{baseline}}$), and sector-by-sector warm-up deltas ($S_1, S_2, S_3$). Visualises an interactive stacked horizontal bar chart (`build_pit_loss_fig`) with phase colouring (Amber for In-Lap Push, Cyan for Pit Lane Transit, Purple for Out-Lap Warm-up), 4 summary KPI cards (Fastest Pit Lane Transit, Best In-Lap Push, Best Out-Lap Warm-up, Grid Median Pit Loss), Head-to-Head sector warm-up breakdown cards, and a full-field classified efficiency leaderboard table with team livery badges (`_render_pit_loss_section`). Automated test suite expanded to 95 tests across 16 modules.
