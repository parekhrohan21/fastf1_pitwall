...
def _render_braking_analysis_section(
    ...
    m1 = _calculate_braking_metrics(win1, apex_dist)
    m2 = _calculate_braking_metrics(win2, apex_dist) if win2 is not None else None
    ...
    if compare and m1 and m2 and m1["initial_brake_dist"] is not None and m2["initial_brake_dist"] is not None:
        delta_m = m1["initial_brake_dist"] - m2["initial_brake_dist"]
        # If delta_m < 0, driver1 braked closer to the apex (braked later)
        if delta_m < -1.0:
            later_drv = drv1_name
            later_col = colour1
            adv_m = abs(delta_m)
            summary_txt = f"<b>{later_drv}</b> initiated braking <b>{adv_m:.1f} m later</b> into {selected_corner_label} than {drv2_name}."
        elif delta_m > 1.0:
            later_drv = drv2_name
            later_col = colour2
            adv_m = abs(delta_m)
            summary_txt = f"<b>{later_drv}</b> initiated braking <b>{adv_m:.1f} m later</b> into {selected_corner_label} than {drv1_name}."
        else:
            later_col = "rgba(255,255,255,0.2)"
            summary_txt = f"Both drivers initiated braking within <b>1.0 m</b> of each other into {selected_corner_label}."

        st.markdown(
            f"<div style='background:rgba(255,255,255,0.03); border-left:4px solid {later_col}; "
